...
def prepare_track(track: APIData, config: Config, albums: AlbumCache) -> PreparedTrack:
    """Download and tag staged audio; the caller validates and publishes it."""
    video_id = track["videoId"]
    filepath = download.download(video_id, config.staging_dir, config.download_timeout)
...
def run_sync(client: MusicClient, playlist_id: str, config: Config, list_only: bool = False) -> int:
    ...
    seen: set[str] = set()
    with closing(history.connect_db(config.db_path, read_only=list_only)) as db:
        for track in tracks:
            ...
            try:
                ...
                result = prepare_track(track, config, albums)
